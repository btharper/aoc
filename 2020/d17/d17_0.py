...
def count_neighbors(cube, x, y, z, w=None):
    count = 0
    for dx in range(-1, 2):
        for dy in range(-1, 2):
            for dz in range(-1, 2):
                if dx == 0 and dy == 0 and dz == 0 and w is None:
                    continue
                if w is None:
        ...
        for x in range(ix-1, ax+2):
            for y in range(iy-1, ay+2):
                for z in range(iz-1, az+2):
                    nbrs = count_neighbors(cube, x, y, z)
            ...
            for y in range(iy-1, ay+2):
                for z in range(iz-1, az+2):
                    for w in range(iw-1, aw+2):
                        nbrs = count_neighbors(cube, x, y, z, w)
