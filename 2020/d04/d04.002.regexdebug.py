...
inp2 = """ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
inp4 = """pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
count = re.compile(
            r'(?:(?:(?:byr|iyr|eyr|hgt|hcl|ecl|pid):\S+)(?:\scid:\S+)?\s?){7}$',
...
valid = re.compile(
            ...
            r'iyr:20(?:1\d|20)|'
            r'eyr:20(?:2\d|30)|'
            r'hgt:1(?:[5-8]\d|9[0-3])cm|'
            r'hgt:(?:59|6\d|7[0-6])in|'
            r'hcl:#[0-9a-f]{6}|'
            r'ecl:(?:amb|blu|brn|grn|gry|hzl|oth)|'
            r'pid:\d{9}'
            r')(?:[ \n]cid:[^:\s]*)?[ \n]?){7}$',
            re.MULTILINE | re.ASCII)

def check(inp):
  oldc, olds = oldcheck(inp)
  ncount, nsecond = 0, 0
  for m in count.finditer(inp):
    #print(m[0])
    ncount += 1
  for m in valid.finditer(inp):
    nsecond += 1
  print('////', ncount, olds)
  return ncount, nsecond
  #return sum(1 for _ in count.finditer(inp)), olds

def oldcheck(inp):
  count = 0
  second = 0
  for pps in inp.split('\n\n'):
    pp = {}
    for field in pps.split():
      k,v = field.split(':')
      pp[k] = v
    if all(req in pp for req in ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid']):
      count += 1
    try:
      ...
      if not re.match('^#[0-9a-f]{6}$', pp['hcl']):
        continue
      if pp['ecl'] not in 'amb,blu,brn,grn,gry,hzl,oth'.split(','):
        continue
      if not re.match('^[0-9]{9}$', pp['pid']):
        continue
      second += 1
    except Exception:
      pass
...
#assert check(inp2)[0] == 2, check(inp2)
#assert check(inp3)[1] == 0, check(inp3)
#assert check(inp4)[1] == 4, check(inp4)

for inp_str in (inp2, inp3, inp4, inp):
  old_c = oldcheck(inp_str)
  new_c = check(inp_str)
  if old_c != new_c:
    for line in inp_str.split('\n\n'):
      old_lc, new_lc = oldcheck(inp_str), check(inp_str)
      print(re.findall(
            r'('
            r'byr:(?:19[2-9]\d|200[0-2])|'
            r'iyr:20(?:1\d|20)|'
            ...
            line,
            re.ASCII))
      assert old_lc == new_lc, (old_lc, new_lc, line)

print(f"d04 sol {check(inp)}")
