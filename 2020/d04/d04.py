...
def check(inp):
  ...
  for passport in inp.split('\n\n'):
    pd = dict(pair.split(':') for pair in passport.split())
    if all(tag in pd for tag in tags):
      p1 += 1
      if (
            (1920 <= int(pd['byr']) <= 2002) and
            (2010 <= int(pd['iyr']) <= 2020) and
            (2020 <= int(pd['eyr']) <= 2030) and
            (pd['ecl'] in val_ecl) and
            (len(pid := pd['pid']) == 9 and int(pid) >= 0) and
            ((hcl := pd['hcl'])[0] == '#' and int(hcl[1:], 16) >= 0)
         ):
        hnum, suf = int(pd['hgt'][:-2]), pd['hgt'][-2:]
        if (150 <= hnum <= 193 and suf == 'cm') or (59 <= hnum <= 76 and suf == 'in'):
          p2 += 1
  return p1, p2
