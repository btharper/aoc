...
def d19(inp):
    ...
    while unmatched:
        ...
        for rule in rules.split('\n'):
            ...
            if all(point in p1_rdict for point in rulesplit):
                ...
                parsed_rule = '(?:' + '|'.join(sorted(chunk_patterns, key=len)) + ')'
                p1_rdict[rule_num] = parsed_rule
                p2_rdict[rule_num] = parsed_rule
            else:
                unmatched = True
    ...
    if do_p2 := '42' in p2_rdict and '31' in p2_rdict and '11' in p2_rdict and p2_rdict['11'] in p2_rdict['0']:
        p2_tail_capture = re.compile(f"(?:{p2_rdict['42']})+?((?:{p2_rdict['31']})+)")
        cnt_pattern = re.compile(f"{p2_rdict['31']}(?=(?:{p2_rdict['31']})*)")
    ...
    for line in messages.split('\n'):
        if p1_pattern.fullmatch(line):
            #print(len(line))
            p1 += 1
        if do_p2 and (m := p2_tail_capture.fullmatch(line)):
            max_cnt = len(tuple(cnt_pattern.finditer(m.group(1))))
            #b_cnt = len(tuple(b_pattern.finditer(m.group(1))))
            #line_cnt = len(tuple(b_pattern.finditer(line)))
            #print(f"{max_cnt=} {b_cnt=} {line_cnt=}")
            for i in range(max_cnt):
                #print(f"(?:{p2_rdict['42']})+\n(?:{p2_rdict['42']}){'{'}{i}{'}'}\n(?:{p2_rdict['31']}){'{'}{i}{'}'}")
                if i == len(re_cache):
                    re_cache.append(re.compile(f"(?:{p2_rdict['42']})+(?:{p2_rdict['42']}){'{'}{i+1}{'}'}(?:{p2_rdict['31']}){'{'}{i+1}{'}'}"))
                if re_cache[i].fullmatch(line):
                    p2 += 1
                    break
            #print(f"\n{p2_rdict['0']}\n\n{m}")
            #part_a = m.group(1)
